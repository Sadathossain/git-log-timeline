...
module.exports = function(program) {
  ...
  function startServer(dataLog) {
    const logStaticPath = path.join(process.env.HOME, '.git-log-timeline');
    ...
    const dataStr = {
      timeline: {
        ...
        type: 'default',
        startDate: currYear,
        text: '<i><span class="c1">Designed</span> by <span class="c2">Sadat Hossain</span></i>',
        date: dataLog
      }
    };
    ...
    console.log('Completed generating log records checked. Log-Static-Path => ', logStaticPath);
  ...
  function gitLog() {
    exec('git log --committer=Sadat --pretty=format:"%an|%ad|%s"', {cwd: cwd}, function(err, data, stderr) {
      if(err) {
